...
/*   Updated: 2023/02/14 17:26:34 by hznagui          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	move(int keycode, t_alldata *a)
{
	...
	if (keycode == 53)
		exit(0);
	else if (((keycode == 0) || (keycode == 123)) && (a->taible[a->i][a->h - 1] != '1'))
	{
		a->taible[a->i][a->h] = '0';
		a->taible[a->i][a->h - 1] = 'P';
		a->d = 0;
	}
	else if (((keycode == 2) || (keycode == 124)) && (a->taible[a->i][a->h + 1] != '1'))
	{
		a->taible[a->i][a->h] = '0';
		a->taible[a->i][a->h + 1] = 'P';
		a->d = 1;
	}
	else if (((keycode == 13) || (keycode == 126)) && (a->taible[a->i - 1][a->h] != '1'))
	{
		a->taible[a->i][a->h] = '0';
		a->taible[a->i - 1][a->h] = 'P';
	}
	else if (((keycode == 1) || (keycode == 125)) && (a->taible[a->i + 1][a->h] != '1'))
	{
		a->taible[a->i][a->h] = '0';
		a->taible[a->i + 1][a->h] = 'P';
	}
...
int	main(int argc, char **argv)
{
	...
	a.wall = mlx_xpm_file_to_image(a.mlx, "../utils/wall.xpm", &a.width,
			&a.height);
	a.ply = mlx_xpm_file_to_image(a.mlx, "../utils/playerfinish.xpm", &a.width,
			&a.height);
	a.opndor = mlx_xpm_file_to_image(a.mlx, "../utils/opendoor.xpm", &a.width,
			&a.height);
	a.ply2 = mlx_xpm_file_to_image(a.mlx, "../utils/playerme9lob.xpm", &a.width,
			&a.height);
	a.win = mlx_new_window(a.mlx, ft_strlen(a.taible[1], '\0') * a.width, a.i
			* a.height, "So_long");
	fill_win(&a);
